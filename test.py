...
#     knowledge_base_db.embedding_search(query_vector=query_vector)
...
if __name__ == "__main__":
    ...
    my_table = db_instance.create_or_open_table(table_name, df=knowledge_base, overwrite=True)
    # my_table.create_index() # 필요시 인덱스 생성

    ## SQL filter
    filter = "date >= '2025-08-05'"
    results = db_instance.filter_sql(filter=filter)
    print("\n--- SQL Results ---")
    print(results)

    ## Vector searching
    results = db_instance.search(query_vector=query_vector, top_k=2)
    print("\n--- Simple Search Results ---")
    print(results)

    ## Vector searching with pre-filter (SQL)
    filtered_results = db_instance.search(query_vector=query_vector, top_k=2, prefilter=filter)
    print("\n--- Filtered Search Results ---")
    print(filtered_results)

    ## Export Parquet
    parquet_file_path = os.path.join(root_folder_path, f'{table_name}.parquet')
    db_instance.export_to_parquet(table_name=table_name, output_path=parquet_file_path)
